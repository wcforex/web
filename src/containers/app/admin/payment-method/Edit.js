...
export default function EditPaymentMethod() {
    ...
    const [name, setName] = useState(pm.pg.name)
    const [account, setAccount] = useState(pm.pg.account)
    const [host, setHost] = useState(pm.pg.host)
    const [minimum, setMinimum] = useState(pm.pg.minimum)
    const [description, setDescription] = useState(pm.pg.description)
    const [status, setStatus] = useState(pm.pg.status)
    ...
    const onUpdate = async (e) => {
        ...
        try {
            const { data } = await instance.patch(`/payment-method/${pm.pg._id}`, payload)
            if (data) {
                setLoading(false)
