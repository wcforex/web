...
export default function EditPackage() {
    ...
    const packagex = location.state
    const navigate = useNavigate()

    const [loading, setLoading] = useState(false)
    const [name, setName] = useState(packagex.pg.name)
    const [duration, setDuration] = useState(packagex.pg.duration)
    const [maxDeposit, setMaxDeposit] = useState(packagex.pg.maxDeposite)
    const [minDeposit, setMinDeposit] = useState(packagex.pg.minDeposite)
    const [dailyInterest, setDailyInterest] = useState(packagex.pg.interestRatePerDay)
    const [totalReturn, setTotalReturn] = useState(packagex.pg.interestRate)
    const [status, setStatus] = useState(packagex.pg.status)
    ...
    const onUpdate = async (e) => {
        ...
        try {
            const { data } = await instance.patch(`/package/${packagex.pg._id}`, payload)
            if (data) {
                setLoading(false)
