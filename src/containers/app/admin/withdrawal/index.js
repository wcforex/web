...
const AllWithdrawals = () => {
    ...
    const fetchData = async () => {
        setLoading('begin')
        try {
            const { data } = await instance.get('/withdrawal/all')
            // console.log(data)
            if (data) {
    ...
    const onEdit = (pg) => {
        navigate(`/admin/withdrawal/${pg._id}`, { state: { pg } })
     }
    return (
                                ...
                                <>
                                    {withdrawals.map((pg, idx) => (
                                        <tr key={idx}>
                                            ...
                                            <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                                {pg.status === 'pending' && <span className='bg-yellow-300 rounded capitalize px-2 py-1'>{pg.status}</span>}
                                                {pg.status === 'paid' && <span className='bg-green-300 rounded capitalize px-2 py-1'>{pg.status}</span>}
                                                {pg.status === 'cancelled' && <span className='bg-red-300 rounded capitalize px-2 py-1'>{pg.status}</span>}
                                                {pg.status === 'failed' && <span className='bg-orange-300 rounded capitalize px-2 py-1'>{pg.status}</span>}
                                            </td>
                                            <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
